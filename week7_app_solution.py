...
1) The Cache API was created to allow Service Workers to have a deeper and fine-grained control over the caching of resources.
...
2) Client/server SQL database engines strive to implement a shared repository of enterprise data. They emphasize scalability, concurrency, centralization, and control. SQLite strives to provide local data storage for individual applications and devices. SQLite emphasizes economy, efficiency, reliability, independence, and simplicity. 
3) expiration of cache set to 36000 seconds, which is good for our purpose, there isnt anything security related, so it being available for 10 hours to view songs should be okay 
4) It is faster due to the cache 
5) a new directory called crime_api_cache.sqlite has been created.
